...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}html{color-scheme:light}body{margin:0;color:var(--ink);background:var(--canvas);font-family:"Yu Gothic",Meiryo,sans-serif;line-height:1.8}
    ...
    .highlight-annotation{padding:1px 0;color:inherit;background:var(--yellow);box-shadow:inset 0 -2px 0 rgba(201,153,0,.24);border-radius:2px}.highlight-annotation.highlight-green{background:var(--green);box-shadow:inset 0 -2px 0 rgba(53,139,69,.24)}.highlight-comment-marker{display:inline-block;margin-left:.3em;color:#755d13;font-family:"Segoe UI Emoji",sans-serif;font-size:20px;line-height:1;vertical-align:-.08em;cursor:pointer}.highlight-comment-marker:hover{transform:scale(1.12)}
...
<body>
  ...
  <script>
    (()=>{
      ...
      document.addEventListener('click',event=>{const marker=event.target.closest('.highlight-comment-marker');if(marker){event.stopPropagation();linkReview(marker.dataset.reviewId);if(activeMarker===marker&&!popover.hidden)closePopover();else openPopover(marker);return}const review=event.target.closest('[data-review-id]');if(review){linkReview(review.dataset.reviewId);return}const side=event.target.closest('[data-sidebar-review-id]');if(side){const id=side.dataset.sidebarReviewId;linkReview(id);document.querySelector('[data-review-id="'+CSS.escape(id)+'"]')?.scrollIntoView({behavior:'smooth',block:'center'});closePopover();return}if(!popover.contains(event.target))closePopover();});
      document.addEventListener('keydown',event=>{if(event.key==='Escape'){closePopover();clearLinked();}});
